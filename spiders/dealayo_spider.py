...
class DealAyoSpider(scrapy.Spider):
    ...
    def productDescriptionParser(self,response):
        ...
        with open('./csv/darazcontinue.json', mode='a') as productsjson:
            data = {
                "Name": str(items['name']),
                "Price": int(Price),
                "ProductURL": "http:"+items['productUrl'],
                "CatagoryName": str(CatagoriesURL["Name"]),
                "DispImage": str(tempImage),
                "Description": str(descriptionText),
                "Images": images
            }
